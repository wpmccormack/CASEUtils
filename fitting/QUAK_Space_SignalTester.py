...
#for b1 in range(1):
    for b2 in range(len(BinList2d[b1])):
        ...
        if("_10000_sigL_-10000_" in BinList2d[-1-b1][-1-b2]):
            print(BinList2d[-1-b1][-1-b2])
            cmd = "python "+templateTop+"/dijetfit.py -i "+BinList2d[-1-b1][-1-b2]+".h5 -M "+str(masspoint)+" --sig_shape "+templateTop+"/"+template+"/graviton_interpolation_M"+str(masspoint)+".0.root --dcb-model -p plots_M"+str(masspoint)+"_"+BinList2d[-1-b1][-1-b2]+" --printSig True -t "+BinList2d[-1-b1][-1-b2]+" -c True --binMin "+str(binMin)+" --binMax "+str(binMax)+" > /dev/null"
            os.system(cmd)
        else:
            print(BinList2d[-1-b1][-1-b2])
            cmd = "python "+templateTop+"/dijetfit.py -i "+BinList2d[-1-b1][-1-b2]+".h5 -M "+str(masspoint)+" --sig_shape "+templateTop+"/"+template+"/graviton_interpolation_M"+str(masspoint)+".0.root --dcb-model -p plots_M"+str(masspoint)+"_"+BinList2d[-1-b1][-1-b2]+" --printSig True -t "+BinList2d[-1-b1][-1-b2]+" --binMin "+str(binMin)+" --binMax "+str(binMax)+" > /dev/null"
            os.system(cmd)

        cmd2 = "combine -M MultiDimFit datacard_JJ_"+BinList2d[-1-b1][-1-b2]+"_FLOAT.txt -m "+str(masspoint)+" -P r -n fit_test_single > /dev/null"
        os.system(cmd2)
        ...
        foundSignal[-1-b1][-1-b2] = signif*100.
            

        #f = h5py.File(BinList2d[b1][b2]+'.h5', 'r')
        #signalRate.SetBinContent(b1+1,b2+1,f['fracSig'][0])
        f = h5py.File(BinList2d[-1-b1][-1-b2]+'.h5', 'r')
        expectedSignal[-1-b1][-1-b2] = f['fracSig'][0]

        

    numHigh = 0
    for b2 in range(len(BinList2d[b1])):
        if(foundSignal[-1-b1][-1-b2] > 1.):
            numHigh += 1

    if(b1==0):
        dc_file = open("combineCommandv2.sh","a")
        for b2 in range(len(BinList2d[b1])):
            dc_file.write('bin_'+BinList2d[-1-b1][-1-b2]+'=datacard_JJ_'+BinList2d[-1-b1][-1-b2]+'.txt ')
        dc_file.close()
...
for b1 in range(len(BinList2d)):
    for b2 in range(len(BinList2d[b1])):
        if(binsChecked < numberQUAKBINs):
            with h5py.File(BinList2d[-1-b1][-1-b2]+".h5", "r") as f:
